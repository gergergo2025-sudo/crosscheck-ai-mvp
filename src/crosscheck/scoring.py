...
class EvidenceScorer:
    ...
    version = "scoring-v3:0.30,0.15,0.20,0.20,0.15:threshold-0.60"
    ...
    def score(self, answers: list[ModelAnswer], *, clustering: ClusteringOutcome,
        ...
        cluster_providers = {
            cluster.id: {
                answer_by_claim[claim_id].provider
                for claim_id in cluster.claim_ids
                if claim_id in answer_by_claim
            }
            for cluster in clustering.clusters
        }
        verified_consensus_clusters = {
            cluster.id
            for cluster in clustering.clusters
            if len(cluster_providers[cluster.id]) >= 2
            and any(
                result.status == "verified"
                for result in verification_by_claim.get(cluster.representative_claim_id, [])
            )
        }
        for answer in answers:
            ...
            authorities = [
                float(item.get("authority", 0))
                for result in fact_results
                if result.status == "verified"
                for item in result.evidence
                if item.get("relation") in {None, "supporting"}
                and isinstance(item.get("authority"), (int, float))
            ]
            ...
            answer_claim_ids = {claim.id for claim in answer.claims if claim.id is not None}
            answer_cluster_ids = {
                cluster.id
                for cluster in clustering.clusters
                if answer_claim_ids.intersection(cluster.claim_ids)
            }
            consensus_total = len(answer_cluster_ids) if usable_provider_count >= 2 else 0
            consensus_passed = len(answer_cluster_ids.intersection(verified_consensus_clusters))
            components = {
            ...
            if answer.parse_status != "parsed":
                ...
                claim_results = [
                    result
                    for claim in answer.claims
                    for result in verification_by_claim.get(claim.id, [])
                ]
                cap_reason: str | None = None
                if any(result.status == "conflict" for result in claim_results):
                    cap_reason = "independent verification conflict"
                elif usable_provider_count < 2:
                    cap_reason = "only one usable provider"
                elif not any(result.status == "verified" for result in claim_results):
                    cap_reason = "no successful independent verification"
                if cap_reason:
                    raw_score = min(raw_score, RECOMMENDATION_THRESHOLD - 0.01)
                    components["assurance_cap"] = {
                        "cap": RECOMMENDATION_THRESHOLD - 0.01,
                        "reason": cap_reason,
                    }
