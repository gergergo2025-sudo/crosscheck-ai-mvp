#!/usr/bin/env bash
# Repository-level regression gate for the CrossCheck AI application repository.
#
# This is the delivered repo's own regression-verification gate, modelled on the
# compound-engineer-workflow contract so Anneal's Regression verification step
# can execute it against this Python + React application. It owns every git /
# network / verdict transcription operation; the Agent performs only the
# semantic recheck ("prepare" -> recheck -> "finalize" / "review-fail").
#
# Unlike the Node/Prisma merge gate that ships with Anneal itself, this gate
# judges the CrossCheck AI codebase with its own deterministic, self-contained
# suite: the Python backend test suite (pytest, async, no external services) and
# the frontend production build. A clean HEAD at the verified commit that passes
# every check produces the exact `MERGE GATE: PASS <oid>` verdict line, which is
# the evidence the chain's merge authorization step requires.
#
# Usage:
#   bash scripts/regression-verification.sh prepare
#   bash scripts/regression-verification.sh finalize
#   bash scripts/regression-verification.sh review-fail '<finding IDs or defect>'
...
persist_refresh_conflict() {
  local pre_head="$1" target_head="$2" conflicts="$3" verdict
  ...
  printf 'REGRESSION REFRESH-CONFLICT: %s\n' "$conflicts"
}
...
# The repository's own gate: run the deterministic, self-contained CrossCheck AI
# suite (Python backend pytest + frontend production build). Emits an exact
# `MERGE GATE: PASS <oid>` line when every check passes at the verified HEAD.
run_repo_gate() {
  local oid="$1" log
  log="$(mktemp "${TMPDIR:-/tmp}/crosscheck-gate.XXXXXX")" \
    || die "cannot create gate output file"
  GATE_LOG="$log"
  {
    echo "CrossCheck AI repository gate for $oid"

    echo "--- backend pytest ---"
    if ! uv run pytest -q 2>&1; then
      echo "MERGE GATE: FAIL (backend pytest)"
      return 1
    fi

    echo "--- python bytecode compile ---"
    if ! uv run python -m compileall -q src 2>&1; then
      echo "MERGE GATE: FAIL (python compileall)"
      return 1
    fi

    echo "--- frontend production build ---"
    if [ -d frontend ]; then
      if ! (cd frontend && npm ci --no-audit --no-fund >/dev/null 2>&1 && npm run build >/dev/null 2>&1); then
        echo "MERGE GATE: FAIL (frontend build)"
        return 1
      fi
    fi

    echo "MERGE GATE: PASS $oid"
    return 0
  } > "$log" 2>&1
  local rc=$?
  GATE_LOG=""
...
finalize() {
  local current latest gate_proof verdict
  read_state
  ...
  if ! run_repo_gate "$current"; then
    gate_proof="$(grep -a -e 'MERGE GATE: ' "$GATE_LOG" 2>/dev/null | tail -1)"
    gate_proof="${gate_proof:-MERGE GATE: FAIL (repository gate)}"
  ...
  gate_proof="MERGE GATE: PASS $current"
